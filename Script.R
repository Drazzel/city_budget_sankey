...
#load data set
data <- read_xlsx("2018 Revenue Sources.xlsx")
...
data2 <- read_xlsx("2018 Expenditures.xlsx")

#condense entries
data2 <- data2 %>% group_by(`CTBA Fund Category`,`Fund Category`,`CTBA Function Category`) %>%
  summarize(`2018 ORDINANCE` = sum(`2018 ORDINANCE`))
...
p5ca <- unique(p5ca)
p5cb <- data.frame(
  fcc= c(data2$`CTBA Function Category`),
...
links3=data2
nodes3=data.frame(name=c(as.character(links3$`Fund Category`), as.character(links3$`CTBA Function Category`)) %>% unique())
links3$IDsource=match(links3$`Fund Category`, nodes3$name)-1
links3$IDtarget=match(links3$`CTBA Function Category`, nodes3$name)-1
#color coding
...
e <- sankeyNetwork(Links = links3, Nodes = nodes3,
                   Source = "IDsource", Target = "IDtarget",
                   Value = "2018 ORDINANCE", NodeID = "name",
                   sinksRight=FALSE, fontSize = 16, units = "$", fontFamily = "Helvetica",
...
links4=data2
nodes4=data.frame(name=c(as.character(links4$`CTBA Fund Category`), as.character(links4$`CTBA Function Category`)) %>% unique())
links4$IDsource=match(links4$`CTBA Fund Category`, nodes4$name)-1
links4$IDtarget=match(links4$`CTBA Function Category`, nodes4$name)-1
#color map
p6ca <- data.frame(
...
data3 <- read_xlsx("2018Master-Sankey.xlsx", sheet = 1)
#sankey begins
links5=data3
...
#load data
data5 <- read_xlsx("2018Master-Sankey.xlsx", sheet = 2)
#begin sankey 
links7=data5
